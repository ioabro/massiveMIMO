...
//     x[tid] = (idx < N) ? input[idx] : 0; // last block may pad with 0's

//     _syncthreads();

//     // Build summation tree over elements.
//     for(int i = blockDim.x/2; i > 0; i = s/2)
//     {
//         if (tid < s)
//             x[tid] += x[tid + i];
...
// __half ac = __hmul(a, c);
// __half bd = __hmul(b, d);
// __half bc = __hmul(b, c);
// __half ad = __hfma(a, d, bc);
